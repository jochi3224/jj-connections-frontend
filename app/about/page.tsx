    ...
    >
      ...
      >
        <div
          ...
          style={{
            background:
              "radial-gradient(400px circle at var(--mouse-x,50%) var(--mouse-y,50%), rgba(180,128,44,0.08), transparent 60%)",
          }}
        />
        ...
        <div className="relative grid gap-8 lg:grid-cols-[0.38fr_0.62fr]">
          ...
          <div>
            ...
            <div className="mt-4 h-px w-0 bg-gradient-to-r from-[var(--gold)] to-transparent transition-all duration-700 group-hover:w-full" />
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <g style={{ transformOrigin: `${cx}px ${cy}px` }}>
        <line ref={hourRef} x1={cx} y1={cy + r * 0.12} x2={cx} y2={cy - r * 0.48}
          stroke="rgba(246,243,238,0.7)" strokeWidth="3" strokeLinecap="round"
          style={{ transformOrigin: `${cx}px ${cy}px`, transition: "transform 0.5s ease" }} />
      </g>
      <g style={{ transformOrigin: `${cx}px ${cy}px` }}>
        <line ref={minuteRef} x1={cx} y1={cy + r * 0.15} x2={cx} y2={cy - r * 0.68}
          stroke="rgba(246,243,238,0.85)" strokeWidth="2" strokeLinecap="round"
          style={{ transformOrigin: `${cx}px ${cy}px`, transition: "transform 0.2s ease" }} />
      </g>
      <g style={{ transformOrigin: `${cx}px ${cy}px` }}>
        <line ref={secondRef} x1={cx} y1={cy + r * 0.2} x2={cx} y2={cy - r * 0.75}
          stroke="rgba(180,128,44,0.9)" strokeWidth="1" strokeLinecap="round"
  ...
  return (
    <svg width="100%" height="100%" viewBox="0 0 120 120" fill="none">
      ...
      {Array.from({ length: 48 }).map((_, i) => {
        ...
        return (
          <line key={i}
            x1={60 + Math.cos(angle) * r1} y1={60 + Math.sin(angle) * r1}
            x2={60 + Math.cos(angle) * r2} y2={60 + Math.sin(angle) * r2}
            stroke="rgba(180,128,44,0.5)" strokeWidth={isMain ? "1" : "0.5"} />
        );
      })}
    </svg>
  );
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} fill="none"
      ...
      {Array.from({ length: 6 }).map((_, i) => {
        ...
        return (
          <line key={i}
            x1={size/2 + Math.cos(angle) * holeR} y1={size/2 + Math.sin(angle) * holeR}
            x2={size/2 + Math.cos(angle) * innerR * 0.65} y2={size/2 + Math.sin(angle) * innerR * 0.65}
            stroke="rgba(180,128,44,0.2)" strokeWidth="1" />
        );
      })}
    </svg>
  );
...
function GoldParticles() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      {particles.map((p) => (
        <div key={p.id} className="absolute rounded-full bg-[var(--gold)]"
          style={{ left: `${p.x}%`, top: `${p.y}%`, width: p.size, height: p.size,
            opacity: p.opacity, animation: `particleMove ${p.duration}s ease-in-out ${p.delay}s infinite` }} />
      ))}
    </div>
  );
}
...
function MarqueeStrip() {
  const words = ["LUXURY WATCHES","·","PRECISION","·","AUTHENTICATION","·","BUY · SELL · TRADE","·","HOROLOGICAL EXCELLENCE","·","ORLANDO FLORIDA","·"];
  ...
  return (
    <div className="overflow-hidden border-y border-[rgba(180,128,44,0.15)] bg-[rgba(180,128,44,0.04)] py-4">
      <div className="flex whitespace-nowrap" style={{ animation: "marquee 30s linear infinite" }}>
        {repeated.map((w, i) => (
          <span key={i} className="mx-6 text-xs font-medium uppercase tracking-[0.3em] text-[var(--gold)]"
  ...
  return (
    <main className="relative">
      ...
      <section className="relative min-h-[92vh] overflow-hidden border-b border-[rgba(180,128,44,0.10)] flex items-center">
        <div className="pointer-events-none absolute right-[-8vw] top-1/2 -translate-y-1/2 opacity-[0.07]"
        ...
        <div ref={heroRef} className="container-luxury relative py-32">
          <div className="flex items-center gap-3 mb-6"
            style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? "translateY(0)" : "translateY(20px)", transition: "opacity 0.6s ease, transform 0.6s ease" }}>
            <div className="h-px w-8 bg-[var(--gold)] opacity-60" />
            <p className="section-kicker">About Us</p>
            <div className="h-px w-8 bg-[var(--gold)] opacity-60" />
          </div>

          <h1 className="max-w-5xl text-5xl font-semibold leading-[1.02] text-[var(--text)] md:text-6xl xl:text-[5.5rem]"
            style={{ fontFamily: "'Georgia', serif", opacity: heroVisible ? 1 : 0,
            ...
            <span className="text-[var(--gold)] relative">
              in luxury watch
              <span className="absolute bottom-0 left-0 h-px bg-gradient-to-r from-[var(--gold)] to-transparent"
                style={{ width: heroVisible ? "100%" : "0%", transition: "width 1.2s cubic-bezier(0.16,1,0.3,1) 0.8s" }} />
            </span>{" "}trading.
          </h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3" style={{ opacity: heroVisible ? 1 : 0, transition: "opacity 0.8s ease 0.5s" }} />

          <div className="mt-14 inline-flex items-center gap-4 rounded-[20px] border border-[rgba(180,128,44,0.2)] bg-[rgba(11,12,14,0.85)] px-5 py-4 backdrop-blur-sm"
      ...
      <section className="py-20 relative overflow-hidden">
        ...
        <div ref={statsRef} className="container-luxury">
          <div className="grid grid-cols-2 gap-6 lg:grid-cols-4">
            {stats.map((stat, i) => (
              <div key={stat.label}
                ...
                <p className="text-5xl font-bold text-[var(--gold-soft)]" style={{ fontFamily: "'Georgia', serif" }}>
                  {statsVisible ? <AnimatedCounter value={stat.value} suffix={stat.suffix} /> : `0${stat.suffix}`}
                </p>
                <p className="mt-2 text-xs uppercase tracking-[0.22em] text-[var(--muted)]">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 relative">
        ...
        <div ref={whoRef} className="container-luxury relative">
          <div
            className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4"
            style={{ opacity: whoVisible ? 1 : 0,
              transform: whoVisible ? "translateY(0)" : "translateY(30px)",
          ...
          >
            {[
              ...
              <article key={item.title}
                className="luxury-card group relative overflow-hidden p-8 cursor-default"
                style={{ opacity: whoVisible ? 1 : 0,
                ...
                <p className="mb-4 flex items-center gap-2 text-xs uppercase tracking-[0.24em] text-[var(--gold)]">
                  <span>{item.icon}</span>
                  <span>0{index + 1}</span>
                </p>
                <h3 className="text-3xl font-semibold text-[var(--text)] group-hover:text-[var(--gold-soft)] transition-colors duration-300"
                  style={{ fontFamily: "'Georgia', serif" }}>
                  {item.title}
                </h3>
                <div className="my-4 h-px w-8 bg-[var(--gold)] opacity-30 group-hover:w-full transition-all duration-500" />
      ...
      <section className="border-y border-[rgba(180,128,44,0.10)] py-28 relative overflow-hidden">
        ...
        <div className="container-luxury relative">
          <div className="mb-16 text-center">
            ...
            <h2 className="section-title" style={{ fontFamily: "'Georgia', serif" }}>
              The people behind
              <br />
            ...
            <div className="mt-6 flex items-center justify-center gap-1">
              {Array.from({ length: 21 }).map((_, i) => (
                <div key={i} className="bg-[var(--gold)]"
                  style={{ width: 1, height: i % 5 === 0 ? 16 : 8, opacity: i % 5 === 0 ? 0.7 : 0.2 }} />
              ))}
            </div>
            <p className="mx-auto mt-5 max-w-2xl text-[var(--muted)]">
              A team driven by passion, technical understanding, and a deep appreciation for luxury timepieces.
            </p>
          </div>
          ...
          <div className="relative overflow-hidden rounded-[40px] border border-[rgba(180,128,44,0.22)] bg-[linear-gradient(135deg,rgba(20,20,22,0.97),rgba(11,12,14,0.99))] p-12 shadow-[0_40px_100px_rgba(0,0,0,0.55)] md:p-16">
            ...
            <div className="relative grid items-center gap-12 lg:grid-cols-[1.3fr_0.7fr]">
              <div>
                ...
                <h2 className="text-4xl font-semibold leading-[1.05] text-[var(--text)] md:text-5xl xl:text-6xl"
                  style={{ fontFamily: "'Georgia', serif" }}>
                ...
                <p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--muted)]">
                  Whether you are looking to buy, sell, trade, polish, or authenticate a luxury watch,
                  JJ Connections is ready to guide you with expertise, clarity, and personalized support.
                </p>
                <div className="mt-8 flex items-center gap-1">
                  {Array.from({ length: 32 }).map((_, i) => (
                    <div key={i} className="bg-[var(--gold)]"
                      style={{ width: 1, height: i % 4 === 0 ? 14 : 7, opacity: i % 4 === 0 ? 0.6 : 0.18 }} />
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 lg:items-end">
                ...
                <Link href="/catalogo" className="outline-button w-full text-center lg:w-auto">
                  Explore Catalog
                </Link>
